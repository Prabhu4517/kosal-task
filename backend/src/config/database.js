...
function initSequelize() {
  ...
  if (isMysql && !env.DB.USE_SQLITE_FALLBACK) {
    ...
    logger.info(
      `Configured SQLite database pool at: ${storagePath} [Development/Testing Mode]`,
    );
...
async function testConnection() {
  try {
    ...
    logger.info(
      `Database connection established successfully. [Dialect: ${sequelize.getDialect()}]`,
    );
    return true;
  } catch (error) {
    logger.error(
      `Unable to connect to the database (${sequelize.getDialect()}): ${error.message}`,
    );
    ...
    if (env.DB.DIALECT === "mysql" && !env.DB.USE_SQLITE_FALLBACK) {
      logger.warn(
        "Attempting SQLite fallback for local development resilience...",
      );
      env.DB.USE_SQLITE_FALLBACK = true;
      sequelize = initSequelize();
      await sequelize.authenticate();
      logger.info(
        "Database connection established successfully via SQLite fallback.",
      );
      return true;
    }
    throw error;
